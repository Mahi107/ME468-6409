...
cyl_s.GetCylinder().p1 = chrono.ChVectorD(0, 0.2, 0);
cyl_s.GetCylinder().p2 = Chrono.ChVectorD(0, -0.2, 0);
cyl_s.GetCylinder().rad = 0.03;
...
chrono.ChQuaternion z2y
chrono.ChQuaternion z2x
...
while (application.GetDevice().run()):
        ...
        array_time.append(systems.GetChTime())
        array_position.append(slider.GetPos().x)
        array_torque.append(engine_ground_crank.GetMotorRot_dt())
    ...
    if system.GetChTime() > 40:
          application.GetDevice().closeDevice()
            
            fig, (ax1, ax2) = plt.subplots(2, sharex = True)
 
ax1.plot(array_time, array_pos)
ax1.set(ylabel='position [m]',xlabel='time')
ax1.grid()
 
ax2.plot(array_time, array_torque, 'r--')
ax2.set(ylabel='torque',xlabel='time')
ax2.grid()
