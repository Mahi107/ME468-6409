...
mean = chrono.ChVectorD(0, 0, 0)
stdv = = chrono.ChVectorD(1.0, 1.0, 1.0)
...
if save:
    cam.PushFilter(sens.ChFilterSave(out_dir + "cam/"))
     ...
     if noise_model == "CONST_NORMAL_XYZI":
        cam.PushFilter(sens.ChFilterCameraNoisePixDep(0, 0.02, 0.02))
...
gps.SetLag(gps_lag)
gps.SetCollectionWindow( 1. / float(gps_update_rate))

# Provides the host access to the gps data
gps.PushFilter(sens.ChFilterGPSAccess())


 if noise_model == "CONST_NORMAL_XYZZ":
        gps.PushFilter(sens.ChGPSNoiseNormal(mean, stdv)
    elif noise_model == "NONE":
        # Don't add any noise models
        pass
...
while (time < tend) :
    ...
    # Advance simulation for one timestep for all module
